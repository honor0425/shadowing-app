...
export default function ShadowingApp() {
  // ── all state in a single ref to avoid closure issues ──
  const S = useRef({
  ...
  const upUI = (patch) => setUI(u => ({ ...u, ...patch }))
  ...
  const initYT = (videoId) => {
    ...
    const tryCreate = () => {
      ...
      ytRef.current = new window.YT.Player('yt-div', {
        ...
        events: {
          onReady: () => {
            S.current.ytReady = true
            upUI({ ytReady: true, status: { dot: '', msg: '就緒，點播放開始' } })
          },
  ...
  // ── core playback (all reads from S.current, no closure issues) ──
  const stopAll = () => {
  ...
  const onSentEnd = () => {
    ...
    if (S.current.repDone < S.current.repN) {
      setTimeout(() => playSentence(S.current.curIdx), 300)
      return
    }
    upUI({ badge: null })
    if (S.current.shadowMode) {
      ...
      S.current.timer = setTimeout(() => {
        S.current.waiting = false
        upUI({ waiting: false, badge: null })
        nextSub()
  ...
  const playSentence = (i) => {
    ...
    if (!sub) { console.log('[playSentence] no sub at', i); return }
    S.current.repDone++
    S.current.curIdx = i
    ...
    if (S.current.tab === 'yt') {
      ...
      if (!yt) { console.log('[playSentence] no yt player'); return }
      if (!S.current.ytReady) { console.log('[playSentence] yt not ready'); return }
      try {
        yt.setPlaybackRate(S.current.speed)
        yt.seekTo(sub.s, true)
        yt.playVideo()
      ...
      } catch(e) { console.log('[playSentence yt] error:', e.message) }
    } else {
      const vid = vidRef.current
      if (!vid) { console.log('[playSentence] no video element'); return }
      vid.playbackRate = S.current.speed
      vid.currentTime = sub.s
      vid.play().catch(e => console.log('[playSentence] play error:', e.message))
      S.current.playing = true
      upUI({ playing: true })
      if (S.current.timer) clearTimeout(S.current.timer)
  ...
  const togglePlay = () => {
    console.log('[togglePlay] subs:', S.current.subs.length, 'tab:', S.current.tab, 'ytReady:', S.current.ytReady, 'playing:', S.current.playing)
  ...
  const loadVideo = (f) => {
    ...
    upUI({ vidSrc: src, showPlayer: true, ytId: '', status: { dot: '', msg: '影片已載入：' + f.name } })
  }
          ...
          {tab==='yt' && <>
            <div>
              ...
              <div className="yt-row">
                <input id="yt-url-input" type="text" placeholder="https://youtube.com/watch?v=..." defaultValue="" onKeyDown={e=>e.key==='Enter'&&loadYT()}/>
          ...
          <div className="player-card">
            {showPlayer&&tab==='local'&&vidSrc
              ? <video ref={vidRef} src={vidSrc}
                  ...
                  onLoadedMetadata={()=>{ const d=vidRef.current?.duration||0; S.current.dur=d; upUI({dur:d,status:{dot:'',msg:'就緒，點播放開始'}}) }}/>
